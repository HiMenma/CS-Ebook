const fs = require("fs");
const path = require("path");

const booksDataPath = "client/src/lib/books-data.ts";
const content = fs.readFileSync(booksDataPath, "utf-8");

const categoryMap = {
  ai: "人工智能",
  programming: "编程语言",
  fundamentals: "计算机基础",
  bigdata: "大数据处理",
  math: "数学基础",
};
const subcategoryMap = {
  ai: {
    "ai-cv": "图像处理",
    "ai-rl": "强化学习",
    "ai-ml": "机器学习",
    "ai-dl": "深度学习",
    "ai-nlp": "自然语言处理",
  },
  programming: {
    "prog-python": "Python",
    "prog-java": "Java",
    "prog-js": "JavaScript",
    "prog-cpp": "C++",
    "prog-go": "Go",
    "prog-rust": "Rust",
  },
  fundamentals: {
    "fund-os": "操作系统",
    "fund-algo": "算法与数据结构",
    "fund-network": "计算机网络",
  },
  bigdata: { "bd-analysis": "数据分析", "bd-mining": "数据挖掘" },
  math: { "math-applied": "应用数学", "math-advanced": "高等数学" },
};

function getAllImageFiles(dir) {
  const files = [];
  const items = fs.readdirSync(dir);
  items.forEach(item => {
    const fullPath = path.join(dir, item);
    if (fs.statSync(fullPath).isDirectory()) {
      files.push(...getAllImageFiles(fullPath));
    } else if (item.endsWith(".jpg")) {
      const relPath = fullPath.replace(/\\/g, "/").replace("images/", "");
      files.push({ relPath: relPath, filename: item.replace(".jpg", "") });
    }
  });
  return files;
}

const imageFiles = getAllImageFiles("images");
const normalizeTitle = title =>
  title.toLowerCase().replace(/\s+/g, " ").replace(/：/g, ":").trim();

const bookRegex =
  /\{\s*id:\s*"([^"]+)",\s*title:\s*"([^"]+)",\s*category:\s*"([^"]+)",\s*subcategory:\s*"([^"]+)"/g;

console.log("缺失封面的书籍及目录文件：\n");
let missingCount = 0;
let match;
while ((match = bookRegex.exec(content)) !== null) {
  const title = match[2];
  const category = match[3];
  const subcategory = match[4];

  const catName = categoryMap[category] || category;
  const subCatName = subcategoryMap[category]?.[subcategory] || subcategory;
  const normalized = normalizeTitle(title);
  const dirPath = catName + "/" + subCatName;

  const filesInDir = imageFiles.filter(f =>
    f.relPath.toLowerCase().startsWith(dirPath.toLowerCase())
  );
  const hasMatch = filesInDir.some(
    f => normalizeTitle(f.filename) === normalized
  );

  if (!hasMatch) {
    missingCount++;
    console.log(missingCount + ". " + title);
    console.log("   期望目录: " + dirPath);
    console.log("   目录文件数量: " + filesInDir.length);
    if (filesInDir.length > 0 && filesInDir.length < 10) {
      filesInDir.forEach(f => console.log("     - " + f.filename));
    }
    console.log("");
  }
}

console.log("总共缺失: " + missingCount + " 本");
